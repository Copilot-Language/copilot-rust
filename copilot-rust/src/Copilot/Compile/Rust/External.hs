...
-- | Represent information about externs needed in the generation of Rust code
-- for stream declarations and triggers.
module Copilot.Compile.Rust.External
    ( External(..)
    , gatherExts
    )
  where
...
-- Internal imports
import Copilot.Compile.Rust.Name ( exCpyName )
...
-- the Rust backend currently only generates code for streams and triggers.
