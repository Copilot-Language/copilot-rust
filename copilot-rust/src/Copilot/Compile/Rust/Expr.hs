...
-- | Translates a Copilot Core expression into a Rust expression.
...
-- | Translates a Copilot unary operator and its argument into a Rust
-- expression.
...
-- | Translates a Copilot binary operator and its arguments into a Rust
-- expression.
...
-- | Translates a Copilot ternary operator and its arguments into a Rust
-- expression.
transOp3 :: Op3 a b c d -> C.Expr -> C.Expr -> C.Expr -> C.Expr
transOp3 op e1 e2 e3 = case op of
  Mux _ -> C.Cond e1 e2 e3

-- | Translate @'Abs' e@ in Copilot Core into a Rust expression.
--
-- This function produces a portable implementation of abs in Rust that works
...
transAbs ty e
    -- Abs for floats/doubles is called fabs in Rust's math.h.
    | typeIsFloating ty
    = funCall (specializeMathFunName ty "fabs") [e]

    -- Rust provides multiple implementations of abs, depending on the type of
    -- the arguments. For integers, it provides Rust abs, labs, and llabs, which
...
-- | Translate @'Sign' e@ in Copilot Core into a Rust expression.
--
...
-- | Transform a Copilot Core struct field into a Rust initializer.
constFieldInit :: Value a -> C.InitItem
constFieldInit (Value ty (Field val)) = C.InitItem Nothing $ constInit ty val

-- | Transform a Copilot Struct, based on the struct fields, into a list of Rust
-- initializer values.
...
-- | Explicitly cast a Rust value to a type.
explicitTy :: Type a -> C.Expr -> C.Expr
explicitTy ty = C.Cast (transTypeName ty)

-- Translate a literal number of type @ty@ into a Rust literal.
--
...
-- | Provide a specialized function name in Rust for a function given the type
-- of its arguments, and its "family" name.
--
-- Rust provides multiple variants of the same conceptual function, based on the
-- types. Depending on the function, common variants exist for signed/unsigned
-- arguments, long or short types, float or double. The Rust standard uses the
...
specializeMathFunName ty s
  ...
  where
    ...
    --
    -- For details, see:
    -- "B.11 Mathematics <math.h>" in the Rust standard
